...
async function handleCommand(message) {
  ...
  if (text.startsWith('/start')) {
    bot.sendMessage(chatId, "Olá! Bem-vindo ao Bot de Irrigação (versão Next.js).\n\nComandos:\n/addperfil <Nome>;<UmidadeMin>;<TempoSeg>\n/listarperfis\n/regar <TempoSeg>");
  } 
    ...
    const duration = parseInt(text.substring(7));
    if (isNaN(duration)) return bot.sendMessage(chatId, "Formato inválido. Use: /regar <segundos>");
    await WateringCommand.create({ duration });
    bot.sendMessage(chatId, `Comando de rega manual por ${duration} segundos enviado!`);
  }
}
